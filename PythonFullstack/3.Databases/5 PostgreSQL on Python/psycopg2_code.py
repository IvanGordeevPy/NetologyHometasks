...
def add_number(cur, numb, id_num):
...
# Требуется заполнить название базы данных, имя пользователя и пароль вручную
if __name__ == '__main__':
    conn = psycopg2.connect(database='', user='',password='')
    with conn.cursor() as cur:
        deletedb(cur)
        createdb(cur)
        add_new_client(cur, 'Alex', 'Alexandrovich', 'AlexAlex@alex.al')
        check(cur)
        add_new_client(cur, 'Ivan', 'Ivanovich', 'IvanIvanovich@ivan.iv')
        check(cur)
        add_number(cur, 88065678392, 123)
        check(cur)
        update_info(cur)
        check(cur)
        drop_number(cur)
